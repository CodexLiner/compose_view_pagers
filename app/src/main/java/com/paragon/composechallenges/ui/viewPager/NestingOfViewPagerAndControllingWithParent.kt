...
fun NestingOfViewPagerAndControllingWithParent() {
    ...
    Column {
        HorizontalPager(
            modifier = Modifier
                .wrapContentSize()
                .weight(0.9f), state = pagerState
        ) {
            when (it) {
                0 , 3, 7 , 9  -> {
                    ParentContentItem(
                        currentChild = { child ->
                            childCurrentPage[pagerState.currentPage] = child
                        },
                        ...
                        content = {
                            AddPets(pageName = pagerState.currentPage , onClick = { count ->
                                childrenCount[pagerState.currentPage] = count
                            })
                        }, pageContent = { a ->
        ...
        Row(
            ...
            Button(onClick = {
                ...
                Log.e("TAG", "NestingOfViewPagerAndControllingWithParent: $childScrollPosition", )
            ...
            Button(onClick = {
                ...
                if (mChildCurrentPage >= 0 && mChildCurrentPage < (childrenCount[pagerState.currentPage] ?: 0) - 1) {
                    ...
                    if (childScrollTo[pagerState.currentPage] == null) {
                        ...
                        childScrollTo[pagerState.currentPage] = childCurrentPage[pagerState.currentPage]?.plus(1)
                    }
                } else {
                    scope.launch {
                        pagerState.animateScrollToPage(pagerState.currentPage + 1)
...
@Preview
@Composable
fun AddPets(onClick: (Int) -> Unit = {}, pageName: Int = 0) {
    Column(
